 ...
 findByEmail: async (id) => {
    ...
    update: async (id, prof) => {
        try {
            ...
            const result = await pool.query(
                'UPDATE PROFISSIONAL SET CPF_PROF = ?, ENDERECO_PROF = ?, NOME_PROF = ? CONTATO_PROF = ?, EMAIL_PROF = ?, SENHA_PROF = ?, AEREA_PROF =?, EXPERIENCIA_PROF=?, ESTADO_PROF = ?, DOCUMENTO_PROF = ?, DATA_NASC = ?, CEP_PROF = ? IMAGEM_PROF WHERE ID_PROF = ?',
                [cpf_prof, endereco_prof, nome_prof, contato_prof, email_prof, senha_prof, estado_prof, documento_prof, data_prof, area_prof, experiencia_prof, cep_prof, id]
            );
